...
def clear_simplified_data_table():
    try:
        ...
        # Load the data from PostgreSQL to make sure the connection works
        df = spark.read.jdbc(url=db_url, table="simplified_data", properties=db_properties)

        # Clone the schema (create an empty DataFrame with the same schema)
        empty_df = df.limit(0)

        # Write the empty DataFrame to the table in overwrite mode
        empty_df.write.jdbc(url=db_url, table="simplified_data", mode="overwrite", properties=db_properties)
